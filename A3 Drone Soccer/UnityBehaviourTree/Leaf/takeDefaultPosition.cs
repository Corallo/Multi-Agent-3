...
public class takeDefaultPosition : Leaf {
    ...
    public override NodeStatus OnBehave (BehaviourState state) {
        ...
        Vector3 ballToGoal = context.self.own_goal.transform.position - context.self.position_ball;
        Vector3 ballToCircle = circle - context.self.position_ball;
        float x_p = ballToGoal.x - offset / 6;
        float c = x_p / ballToCircle.x;

        Vector3 target = context.self.position_ball + c * ballToCircle;
